...
fn main() -> eframe::Result<()> {
    ...
    // Safety flag to prevent listening to self-generated clicks
    let is_simulating = Arc::new(AtomicBool::new(false));
    ...
    // 1. Background Thread: Click Loop
    {
        ...
        let simulating = Arc::clone(&is_simulating);

        thread::spawn(move || loop {
            if running.load(Ordering::SeqCst) {
                // Signal to global listener thread to ignore incoming fake clicks
                ...
                // Small yield to allow kernel input queue to flush
    ...
    // 2. Background Thread: Global Listener (Hotkeys & Mouse Picker)
    {
        ...
        thread::spawn(move || {
            let callback = move |event: Event| {
                // IGNORE SYNTHETIC CLICKS GENERATED BY OUR OWN CLICK LOOP
